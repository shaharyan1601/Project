...
const OrganizationChartBodyHR = ({ peopleDetails }) => (
...
export default OrganizationChartBodyHR;
  ...
  <div className="organizationChartList_position">
    ...
    >
      {peopleDetails.map((peopleDetail) => (
        <PeopleInfo peopleDetails={peopleDetail} />
      ))}
    </div>
  </div>
);

const PeopleInfo = ({ peopleDetails }) => {
  const { name, profilePhoto, employeePosition } = peopleDetails;

  return (
    <NavLink to="/PeopleInfoHR">
      <div
